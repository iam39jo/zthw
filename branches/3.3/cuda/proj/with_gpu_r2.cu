...
__constant__ struct axis cudaPtr[4096];
...
__global__ void threadCode(int count, float radius2, float *result)
{
	int thId;
	int i;
	...
	for (i = thId + 1; i < count; i++) {
		if (distance(&cudaPtr[thId], &cudaPtr[i]) <= radius2) {
			result[thId] += cudaPtr[i].v;
			result[i] += cudaPtr[thId].v;
		}
	}
}
...
int paralize(int count, float radius, struct axis *points, float *sum)
{
	...
	cudaMemcpyToSymbol(cudaPtr, points, sizeof(struct axis)*count);
	cudaMemset(cudaRst, 0x0, sizeof(float)*count);

	dim3 dimBlock(BLOCK_SIZE, BLOCK_SIZE);
	dim3 dimGrid((count+BLOCK_SCALE-1)/BLOCK_SCALE, 1);

	threadCode<<<dimGrid, dimBlock>>>(count, radius*radius, cudaRst);
