...
__device__ void device_MEMSET(int size,int* arr,int value,int tid) {
	int i,j,k,step;
...
__global__ void CALC(float4* newP,float* ans,int* count,int* begin,int gid) {
	...
	device_MEMSET(readnum,BEG,0,tid);
	if(tid==0) 	{
		for(i=0;i<readnum;i++) {
...
void N_BODY(int _N, const float4* P)
{
	...
	CALC<<<blocknum,threadnum>>>(newP,ans,count,begin,0);
