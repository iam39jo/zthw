...
__global__ void SORT_1(int N,int* count,int* begin,int* belong,int* retrive,float4* oldP,float4* newP,
	...
	for(i=beg;i<N && i<end;i++) {
		...
		belong[i]=ID;
	//	begin[ID]++; count[ID]++;
		atomicAdd(&(begin[ID]),1);	atomicAdd(&(count[ID]),1);
	}
}
...
__global__ void SORT_3(int N,int* count,int* begin,int* belong,int* retrive,float4* oldP,float4* newP,
	...
	for(i=beg;i<N && i<end;i++) { 
		ID=belong[i]; 
	//	j=--begin[ID];
		j=atomicSub(&(begin[ID]),1)-1;
		retrive[j]=i; newP[j]=oldP[i]; }
}
//SORT_2 collecting datas. only need to run on 1 thread
int partical_count_CPU[MAX];
...
__global__ void RESORT(int N,int* partical_retrive,float* _ANS,float* ans,int threadnum,int blocknum) {
	int i,j,k;
	...
	id=bid*threadnum+tid;
	step=ceil(float(N)/blocknum/threadnum);
	for(i=id*step;i<N && i<(id+1)*step;i++) _ANS[partical_retrive[i]]=ans[i];
}
//
...
void N_BODY(int _N, float4* P,int threadnum,int blocknum,int gridnum)
{
	...
	cudaMemcpy(partical_count_CPU,partical_count,N*sizeof(int),cudaMemcpyDeviceToHost);
	for(i=0;i<gridnum;i++) {
		partical_threadnum=-1;
		for(j=i*blocknum;j<(i+1)*blocknum && j<partical_cubenum;j++) 
			if(partical_count_CPU[j]>partical_threadnum) partical_threadnum=partical_count_CPU[j];
		if(partical_threadnum>0)
...
void input() {
	int i,j,k;
	FILE* file=fopen("input_10kk.txt","r");
	if(!file) printf("cannot open the specified file!");
	fscanf(file,"%d%f",&N,&R);
...
{
	sscanf(argv[0],"%d",&blocknum);
	sscanf(argv[1],"%f",&r);
	sscanf(argv[2],"%f",&partical_r);
	int i;
